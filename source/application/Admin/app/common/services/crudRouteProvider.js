	...
	function crudRouteProvider($routeProvider) {
		...
		this.routesFor = function (resourceName, urlPrefix, routePrefix) {
			...
			var templateUrl = function (operation) {
				return baseUrl + '/' + resourceName.toLowerCase() + '/' + operation.toLowerCase() + '.cshtml';
			};
			...
			var controllerName = function (operation) {
				return resourceName + operation + 'Controller';
			};
			...
			// creating CRUD routes.  Specifically we have `whenList(), `whenAdd()` and `whenEdit()`.
			var routeBuilder = {
				...
				whenAdd: function (resolveFns) {
					routeBuilder.when(baseRoute + '/add', {
						templateUrl: templateUrl('add'),
						controller: controllerName('Add'),
						resolve: resolveFns
					});
					return routeBuilder;
				},
